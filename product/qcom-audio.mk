# Audio
PRODUCT_COPY_FILES += \
	 $(LOCAL_PATH)/audio/acdb/MTP/msm8939-tapan-snd-card/MTP_WCD9306_Bluetooth_cal.acdb:system/etc/acdb/MTP/msm8939-tapan-snd-card/MTP_WCD9306_Bluetooth_cal.acdb \
    $(LOCAL_PATH)/audio/acdb/MTP/msm8939-tapan-snd-card/MTP_WCD9306_General_cal.acdb:system/etc/acdb/MTP/msm8939-tapan-snd-card/MTP_WCD9306_General_cal.acdb \
    $(LOCAL_PATH)/audio/acdb/MTP/msm8939-tapan-snd-card/MTP_WCD9306_Global_cal.acdb:system/etc/acdb/MTP/msm8939-tapan-snd-card/MTP_WCD9306_Global_cal.acdb \
    $(LOCAL_PATH)/audio/acdb/MTP/msm8939-tapan-snd-card/MTP_WCD9306_Handset_cal.acdb:system/etc/acdb/MTP/msm8939-tapan-snd-card/MTP_WCD9306_Handset_cal.acdb \
    $(LOCAL_PATH)/audio/acdb/MTP/msm8939-tapan-snd-card/MTP_WCD9306_Hdmi_cal.acdb:system/etc/acdb/MTP/msm8939-tapan-snd-card/MTP_WCD9306_Hdmi_cal.acdb \
    $(LOCAL_PATH)/audio/acdb/MTP/msm8939-tapan-snd-card/MTP_WCD9306_Headset_cal.acdb:system/etc/acdb/MTP/msm8939-tapan-snd-card/MTP_WCD9306_Headset_cal.acdb \
    $(LOCAL_PATH)/audio/acdb/MTP/msm8939-tapan-snd-card/MTP_WCD9306_Speaker_cal.acdb:system/etc/acdb/MTP/msm8939-tapan-snd-card/MTP_WCD9306_Speaker_cal.acdb \
    $(LOCAL_PATH)/audio/acdb/MTP/MTP_Bluetooth_cal.acdb:system/etc/acdbdata/MTP/MTP_Bluetooth_cal.acdb \
    $(LOCAL_PATH)/audio/acdb/MTP/MTP_General_cal.acdb:system/etc/acdbdata/MTP/MTP_General_cal.acdb \
    $(LOCAL_PATH)/audio/acdb/MTP/MTP_Global_cal.acdb:system/etc/acdbdata/MTP/MTP_Global_cal.acdb \
    $(LOCAL_PATH)/audio/acdb/MTP/MTP_Handset_cal.acdb:system/etc/acdbdata/MTP/MTP_Handset_cal.acdb \
    $(LOCAL_PATH)/audio/acdb/MTP/MTP_Hdmi_cal.acdb:system/etc/acdbdata/MTP/MTP_Hdmi_cal.acdb \
    $(LOCAL_PATH)/audio/acdb/MTP/MTP_Headset_cal.acdb:system/etc/acdbdata/MTP/MTP_Headset_cal.acdb \
    $(LOCAL_PATH)/audio/acdb/MTP/MTP_Speaker_cal.acdb:system/etc/acdbdata/MTP/MTP_Speaker_cal.acdb \
	 $(LOCAL_PATH)/audio/acdb/QRD/msm8939-snd-card-skul/QRD_SKUL_WCD9306_Bluetooth_cal.acdb:system/etc/acdb/QRD/msm8939-snd-card-skul/QRD_SKUL_WCD9306_Bluetooth_cal.acdb \
    $(LOCAL_PATH)/audio/acdb/QRD/msm8939-snd-card-skul/QRD_SKUL_WCD9306_General_cal.acdb:system/etc/acdb/QRD/msm8939-snd-card-skul/QRD_SKUL_WCD9306_General_cal.acdb \
    $(LOCAL_PATH)/audio/acdb/QRD/msm8939-snd-card-skul/QRD_SKUL_WCD9306_Global_cal.acdb:system/etc/acdb/QRD/msm8939-snd-card-skul/QRD_SKUL_WCD9306_Global_cal.acdb \
    $(LOCAL_PATH)/audio/acdb/QRD/msm8939-snd-card-skul/QRD_SKUL_WCD9306_Handset_cal.acdb:system/etc/acdb/QRD/msm8939-snd-card-skul/QRD_SKUL_WCD9306_Handset_cal.acdb \
    $(LOCAL_PATH)/audio/acdb/QRD/msm8939-snd-card-skul/QRD_SKUL_WCD9306_Hdmi_cal.acdb:system/etc/acdb/QRD/msm8939-snd-card-skul/QRD_SKUL_WCD9306_Hdmi_cal.acdb \
    $(LOCAL_PATH)/audio/acdb/QRD/msm8939-snd-card-skul/QRD_SKUL_WCD9306_Headset_cal.acdb:system/etc/acdb/QRD/msm8939-snd-card-skul/QRD_SKUL_WCD9306_Headset_cal.acdb \
    $(LOCAL_PATH)/audio/acdb/QRD/msm8939-snd-card-skul/QRD_SKUL_WCD9306_Speaker_cal.acdb:system/etc/acdb/QRD/msm8939-snd-card-skul/QRD_SKUL_WCD9306_Speaker_cal.acdb \
	 $(LOCAL_PATH)/audio/acdb/QRD/QRD_Bluetooth_cal.acdb:system/etc/acdbdata/QRD/QRD_Bluetooth_cal.acdb \
    $(LOCAL_PATH)/audio/acdb/QRD/QRD_General_cal.acdb:system/etc/acdbdata/QRD/QRD_General_cal.acdb \
    $(LOCAL_PATH)/audio/acdb/QRD/QRD_Global_cal.acdb:system/etc/acdbdata/QRD/QRD_Global_cal.acdb \
    $(LOCAL_PATH)/audio/acdb/QRD/QRD_Handset_cal.acdb:system/etc/acdbdata/QRD/QRD_Handset_cal.acdb \
    $(LOCAL_PATH)/audio/acdb/QRD/QRD_Hdmi_cal.acdb:system/etc/acdbdata/QRD/QRD_Hdmi_cal.acdb \
    $(LOCAL_PATH)/audio/acdb/QRD/QRD_Headset_cal.acdb:system/etc/acdbdata/QRD/QRD_Headset_cal.acdb \
    $(LOCAL_PATH)/audio/acdb/QRD/QRD_Speaker_cal.acdb:system/etc/acdbdata/QRD/QRD_Speaker_cal.acdb \
    $(LOCAL_PATH)/audio/audio_platform_info.xml:system/etc/audio_platform_info.xml \
    $(LOCAL_PATH)/audio/audio_policy.conf:system/etc/audio_policy.conf \
    $(LOCAL_PATH)/audio/mixer_paths.xml:system/etc/mixer_paths.xml \
    $(LOCAL_PATH)/audio/mixer_paths_mtp.xml:system/etc/mixer_paths_mtp.xml \
    $(LOCAL_PATH)/audio/mixer_paths_skuh.xml:system/etc/mixer_paths_skuh.xml \
    $(LOCAL_PATH)/audio/mixer_paths_skuhf.xml:system/etc/mixer_paths_skuhf.xml \
    $(LOCAL_PATH)/audio/mixer_paths_skui.xml:system/etc/mixer_paths_skui.xml \
    $(LOCAL_PATH)/audio/mixer_paths_skuk.xml:system/etc/mixer_paths_skuk.xml \
    $(LOCAL_PATH)/audio/mixer_paths_skul.xml:system/etc/mixer_paths_skul.xml \
    $(LOCAL_PATH)/audio/mixer_paths_wcd9306.xml:system/etc/mixer_paths_wcd9306.xml \
    $(LOCAL_PATH)/audio/mixer_paths_sbc.xml:system/etc/mixer_paths_sbc.xml

# Permissions
PRODUCT_COPY_FILES += \
    frameworks/native/data/etc/android.hardware.audio.low_latency.xml:system/etc/permissions/android.hardware.audio.low_latency.xml

# Properties
PRODUCT_PROPERTY_OVERRIDES += \
    ro.qc.sdk.audio.fluencetype=fluence \
    persist.audio.fluence.voicecall=true \
    persist.audio.fluence.voicerec=false \
    persist.audio.fluence.speaker=false
